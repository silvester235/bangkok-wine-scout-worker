...
function openDelete(events,bulk){state.targets=events;state.bulk=bulk;$('target').textContent=bulk?events.length+' selected events':(events[0].title||'Untitled event')+' · '+safeDate(events[0].eventDate);confirmText.value='';confirmDelete.disabled=true;dialog.showModal();confirmText.focus()}
confirmText.addEventListener('input',()=>confirmDelete.disabled=confirmText.value!=='DELETE');$('confirm-form').addEventListener('submit',async event=>{if(event.submitter?.value!=='default')return;if(confirmText.value!=='DELETE'||!state.targets.length){event.preventDefault();return}event.preventDefault();const targets=[...state.targets],ids=targets.map(item=>item.id);confirmDelete.disabled=true;confirmDelete.textContent='Deleting…';try{const response=state.bulk?await fetch('/admin/events/bulk-delete',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({eventIds:ids})}):await fetch('/admin/events/'+encodeURIComponent(ids[0]),{method:'DELETE'});if(response.status===401){location.href='/admin/events-ui';return}const result=await response.json().catch(()=>null);if(!response.ok)throw new Error(result?.error?.message||'Deletion could not be completed.');const removed=state.bulk?(result.results||[]).filter(item=>item?.success).map(item=>item.eventId):(result?.success?ids:[]);if(!removed.length)throw new Error(result?.error?.message||'No selected events were deleted.');const removedSet=new Set(removed);state.events=state.events.filter(item=>!removedSet.has(item.id));for(const id of removed)state.selected.delete(id);dialog.close();render();if(state.bulk&&result.failed)toast(removed.length+' removed; '+result.failed+' failed and remain selected.',true);else toast(removed.length===1?'Event deleted permanently.':removed.length+' events deleted permanently.')}catch(error){toast(error instanceof Error?error.message:'Network interruption. Please retry.',true)}finally{confirmDelete.textContent='Delete permanently';confirmDelete.disabled=confirmText.value!=='DELETE'}});
async function load(){rows.innerHTML='<tr><td class="loading" colspan="9">Loading events…</td></tr>';try{const response=await fetch('/admin/events');if(response.status===401){location.href='/admin/events-ui';return}const body=await response.json();if(!response.ok||!Array.isArray(body.events))throw new Error(body?.error?.message||'Malformed event data.');state.events=body.events.filter(e=>e&&typeof e.id==='string');const ids=new Set(state.events.map(e=>e.id));for(const id of state.selected)if(!ids.has(id))state.selected.delete(id);render()}catch(error){rows.innerHTML='';const tr=document.createElement('tr'),td=text('td',error instanceof Error?error.message:'Could not load events.','error-state');td.colSpan=9;tr.append(td);rows.append(tr);shown.textContent='Listing unavailable';toast('Failed to load events.',true)}}
selectAll.addEventListener('change',()=>{for(const event of filtered())selectAll.checked?state.selected.add(event.id):state.selected.delete(event.id);render()});bulkDelete.addEventListener('click',()=>{const events=state.events.filter(event=>state.selected.has(event.id));if(events.length)openDelete(events,true)});for(const id of ['search','status','sort'])$(id).addEventListener(id==='search'?'input':'change',render);$('refresh').addEventListener('click',load);load()})();
